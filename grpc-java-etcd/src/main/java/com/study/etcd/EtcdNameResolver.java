			...
			case PUT:
				...
				try {
					...
					logger.log(Level.WARNING,
							String.format("ignoring invalid uri. dir='%s', addr='%s'", serviceDir, addr), e);
				}
				break;
			case DELETE:
				addr = event.getKeyValue().getKey().toString(Charsets.UTF_8);
	...
	private void initializeAndWatch() {
		...
		for (KeyValue kv : query.getKvs()) {
			...
			try {
				...
				logger.log(Level.WARNING,
						String.format("Unable to parse server address. dir='%s', addr='%s'", serviceDir, addr), e);
			}
		}

		updateListener();

		// set the Revision to avoid race between initializing URIs and watching for
		// changes.
		WatchOption options = WatchOption.newBuilder().withRevision(query.getHeader().getRevision()).build();
